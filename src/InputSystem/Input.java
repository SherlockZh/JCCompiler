...
public class Input {
	...
	private int         nextChar = END; //指向当前要读入的字符位置
	...
	private boolean noMoreChars() {
		 ...
		 * 缓冲区中是否还有可读的字符
		 */
		return (Eof_read && nextChar >= END_BUF);
	}
	...
	public String preText() {
		byte[] str = Arrays.copyOfRange(START_BUF, preMark, preMark + getPreLength());
	...
	public String text() {
		byte[] str = Arrays.copyOfRange(START_BUF, startMark, startMark + getCurLength());
		return new String(str, StandardCharsets.UTF_8);
	}
	...
	public byte advance() {
		...
		if (START_BUF[nextChar] == '\n') {
			LineNum++;
		}

		return START_BUF[nextChar++];
	}
	...
	private int flush(boolean force) {
		 ...
		 *                            preMark                     DANGER
		 *                              |                          |
		 *     START_BUF              startMark      endMark       | nextChar  END_BUF
		 *         |                    | |           |            |  |      |
		 *         V                    V V           V            V  V      V
		 *         +---------------------------------------------------------+---------+
	...
	private int fillBuffer(int starting_at) {
		...
		if (need < 0) {
			System.err.println("Internal Error (fill buffer): Bad read-request starting addr.");
		}
	...
	public boolean pushback(int n) {
		 ...
		 * 把预读取的若干个字符退回缓冲区
		 */
		while (--n >= 0 && nextChar > startMark) {
			if (START_BUF[--nextChar] == '\n' || START_BUF[nextChar] == '\0') {
				--LineNum;
			}
		}
	...
	public byte lookahead(int n) {
		 ...
		 * 预读取若干个字符
		 */
		byte p = START_BUF[nextChar + n - 1];
//		if (Eof_read && nextChar + n - 1 >= END_BUF) {
//			return EOF;
//		}
		
		return (nextChar + n - 1 < 0 || nextChar + n - 1 >= END_BUF) ? 0 : p;
	}
}
